...
def visualize_and_save_tsne(
    ...
    # New example data
    new_example = avg_over_dim(new_sample, axis=2)  # Assuming new_sample is defined
    new_example_tsne = tsne.fit_transform(np.vstack([combined_samples, new_example]))[-1]

    # Add the new example to the DataFrame
    new_example_df = pd.DataFrame(
        {
            "tsne_1": [new_example_tsne[0]],
            "tsne_2": [new_example_tsne[1]],
            "sample_type": ["new_example"]
        }
    )
    tsne_df = pd.concat([tsne_df, new_example_df], ignore_index=True)

    # Plot the t-SNE samples
    plt.figure(figsize=(8, 8))
    for sample_type, color in zip([samples1_name, samples2_name, "new_example"], ["red", "blue", "green"]):
