...
def visualize_and_save_tsne(
    ...
    min_ = min(min(samples1.shape[0], samples2.shape[0]), min(samples3.shape[0], samples4.shape[0]))
    ...
    samples3_2d = avg_over_dim(samples3, axis=2)
    samples4_2d = avg_over_dim(samples4, axis=2)
    ...
    # Combine the original and generated samples
    combined_samples = np.vstack(
        [samples1_2d[:used_samples], samples2_2d[:used_samples], samples3_2d[:used_samples], samples4_2d[:used_samples]]
    )

    # Compute the t-SNE of the combined samples
    tsne = TSNE(n_components=2, perplexity=40, n_iter=300, random_state=42)
    #tsne = TSNE(n_components=3, perplexity=40, early_exaggeration=25, n_iter=300, random_state=42)
    tsne_samples = tsne.fit_transform(combined_samples)
    ...
    tsne_df = pd.DataFrame(
        {
            ...
            "tsne_2": tsne_samples[:, 1],
            "sample_type": [samples1_name] * used_samples
            + [samples2_name] * used_samples + [samples3_name] * used_samples + [samples4_name] * used_samples,
        }
    )

    # Plot the t-SNE samples
    plt.figure(figsize=(8, 8)) # ["grey", "yellow", "red", "blue"]
    for sample_type, color in zip([samples1_name, samples2_name, samples3_name, samples4_name], ["white", "white", "red", "blue"]):
        if sample_type is not None:
            indices = tsne_df["sample_type"] == sample_type
            plt.scatter(
                tsne_df.loc[indices, "tsne_1"],
